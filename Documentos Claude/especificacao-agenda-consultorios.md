...
**Conteúdo da anamnese importada — 2026-08-04**: o texto gravado em `Paciente.anamnese` na importação (`src/lib/importacao.ts`) agora começa com os 13 campos cadastrais da planilha (`Nome Completo`, `Data de Nascimento`, `Estado Civil`, `Nacionalidade`, `Seu Instagram`, `E-mail`, `Endereço Completo`, `CEP`, `Profissão`, `Telefone (WhatsApp)`, `Seu RG`, `Seu CPF`, `Quem indicou?`), na ordem em que aparecem na planilha — antes esses campos só viravam coluna própria do `Paciente`, nunca apareciam no texto que a profissional lê. Em seguida vêm as perguntas de saúde/anamnese propriamente ditas, também na ordem da planilha, e por fim o separador `--- OBSERVAÇÕES ---` (inalterado — continua sendo onde a clínica escreve manualmente depois). Metadados de submissão do forms.app (`Submitter`, `Submission Date`, `Submission ID`, `Idade`, e qualquer coluna futura que comece com "submission") nunca entram no texto. Escopo da mudança de 2026-08-04 foi só a montagem do texto — a gravação em `POST /api/importacao/executar` persistia só `nome`/`telefone`/`email`/`cpf`/`logradouro`/`cep`/`quemIndicou` como campo próprio do `Paciente`.

**Persistência dos campos cadastrais em coluna própria — 2026-08-06**: `rg`/`dataNascimento`/`estadoCivil`/`nacionalidade`/`profissao`/`instagram` passaram a ser gravados também nas colunas próprias do `Paciente` na importação (em adição ao texto da anamnese, que continua idêntico) — antes só existiam dentro do texto, nunca em coluna, o que fazia o modal de edição abrir esses campos vazios mesmo quando o dado existia (achado investigando o caso da Thaís Siqueira). Célula vazia/ausente na planilha grava `null`, nunca `""` (contrato de três estados, reaproveitado do `PATCH /api/pacientes/[id]`). `dataNascimento` é gravado sem parse (`Paciente.dataNascimento` é `String?`, formato brasileiro `DD/MM/AAAA` esperado mas não convertido — mesmo padrão do cadastro manual); fora desse formato, grava do mesmo jeito e avisa no log da importação. Sem backfill dos pacientes já importados antes desta correção. Detalhe técnico completo em `ARCHITECTURE.md` §9.
...
- **Alerta de conexão perdida + checagem noturna (2026-07-24)**: um incidente real de token do Google revogado/expirado (`invalid_grant`) motivou dois mecanismos novos, complementares ao que já existia. Primeiro, a saúde real da conexão (`Clinica.googleTokenValido`) passa a ser detectada de verdade — antes, `googleConectado` só refletia se alguém clicou em conectar/desconectar, nunca se o token realmente ainda funcionava. Quando cai, um **banner persistente** aparece no painel, visível em qualquer aba, não só um aviso que some em 4 segundos. Segundo, uma **checagem noturna automática** (uma vez por dia, de madrugada) compara o banco contra o Google de verdade e sinaliza (sem corrigir sozinha) sessões futuras cujo evento sumiu ou está com horário diferente do que o painel mostra — o mesmo tipo de problema que já apareceu manualmente nos casos do Jadir/Maura/Fábio, agora detectado sem precisar de auditoria manual.
- **Fila de sincronização com retry automático (2026-08-14)**: até então, se a chamada ao Google falhasse na hora (rede instável, quota, escopo insuficiente — qualquer coisa além de token revogado), a tentativa se perdia — sem repetir sozinha, sem sinal pra ninguém além de um log técnico. Agora, criar/mover/cancelar sessão e criar a pasta do paciente no Drive gravam o dado no banco normalmente e **enfileiram** a parte do Google numa fila própria; um processo automático (a cada 10 minutos) tenta de novo sozinho, com espera crescente entre tentativas (1min, 5min, 30min, 2h, 6h) até 5 vezes — se ainda assim não conseguir, o item fica marcado como falha permanente e a operadora vê um contador em Configurações → Integrações, com um botão "Tentar de novo". O banner de alerta no painel passa a acender também quando essa fila teve uma falha recente (não só quando o token cai de vez). Cancelar uma sessão passou a também cancelar, na mesma operação, qualquer tentativa de criação/atualização daquela mesma sessão que ainda estivesse esperando na fila — sem isso, uma sessão cancelada podia "ressuscitar" no Google minutos depois, porque uma tentativa antiga da fila ainda não tinha rodado. **Compartilhar pasta + enviar e-mail de boas-vindas continua imediato (não entra nessa fila)**: é uma ação que a operadora faz olhando a tela, decidindo o texto na hora — se falhar, ela vê e decide se tenta de novo ali mesmo; enfileirar isso arriscaria mandar o e-mail duas vezes sem ela saber. O acesso do sistema ao Google Drive continua restrito só às pastas que o próprio sistema cria (nunca o Drive inteiro da clínica) — a fila de retry não muda isso.
- **Aviso de reconexão em qualquer tela (2026-08-14)**: quando a conexão com o Google cai, um aviso passa a aparecer na hora, em qualquer página do sistema (não só quem entra em Configurações percebe). Quem administra a integração (ADMIN/PROFISSIONAL) vê um botão "Conectar agora" que reabre a autorização do Google numa janela separada, sem perder a tela em que estava — ao terminar, a janela fecha sozinha e o aviso some se resolveu. Quem não administra a integração (OPERADOR) não vê esse botão — em vez disso, um botão "Avisar responsável" manda uma mensagem de WhatsApp pra quem cuida da conta (mesmo número que já recebe avisos de "atendimento humano necessário" hoje). O aviso não trava a tela — dá pra fechar e continuar trabalhando — mas volta a aparecer na próxima página enquanto a conexão continuar caída. Continua valendo a regra de sempre: só quem administra pode reconectar, e reconectar vincula a conta Google de quem clicou — a clínica depende de uma conta específica (hoje, `contato@pamelarachid.com.br`) ficar conectada.
