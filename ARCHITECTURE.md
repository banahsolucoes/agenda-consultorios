...
| GET | `/api/cron/whatsapp-lembretes` | `cron/whatsapp-lembretes/route.ts` | Envia template `confirmacao_agenda` (Meta) para sessões ~48h à frente ainda não confirmadas (2026-07-24) — ver seção 10 | `CRON_SECRET` |
| GET | `/api/cron/whatsapp-dia-sessao` | `cron/whatsapp-dia-sessao/route.ts` | Envia o link do Meet (texto livre) pras sessões de hoje que já têm link, dentro da janela de 24h (2026-07-24) — ver seção 10 | `CRON_SECRET` |
...
- **Achado (2026-07-24) — limite de cron do plano Vercel Hobby**: além do já documentado "1x/dia por cron" (seção 9), o plano Hobby também limita o **total de cron jobs por projeto a 2**. Com a adição do 3º cron (`whatsapp-dia-sessao`, abaixo), o projeto passou a ter 3 — testado a pedido do usuário para confirmar se a Vercel bloqueia; ver resultado do deploy registrado no commit correspondente. Se bloquear, a correção é fundir `whatsapp-dia-sessao` dentro de `whatsapp-lembretes` (mesma execução diária, checando os dois critérios) em vez de manter rotas separadas.
