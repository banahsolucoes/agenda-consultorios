...
- **PENDENTE DE DECISÃO (2026-08-04) — tratamento de duplicidade/complemento na importação**: hoje `lerEDeduplicarPlanilha()` marca `status: "existente"` quando o CPF (normalizado) já está entre os pacientes da clínica, e `POST /api/importacao/executar` só processa `status === "novo"` — paciente existente é **ignorado silenciosamente**: nenhum campo cadastral é atualizado, nenhuma anamnese nova é anexada, mesmo que a planilha tenha dado novo (reenvio de formulário, complemento de cadastro). Isso é intencional pra nunca sobrescrever o que a clínica já editou manualmente na ficha, mas tem custo: informação nova de um reenvio real nunca chega ao paciente.
  - **RISCO CONHECIDO**: o dedupe por CPF só funciona quando `cpfDigitos.length > 0` (`src/lib/importacao.ts`, `jaExiste = cpfDigitos.length > 0 && cpfsExistentes.has(cpfDigitos)`) — uma linha da planilha com CPF em branco **nunca** é marcada como `"existente"`, sempre entra como `"novo"`. Reimportar a mesma pessoa sem CPF preenchido cria paciente duplicado. Há registros nessa condição na base (49 pacientes sem CPF na Pâmela — subconjunto real, não hipotético — ver dry-run acima).
  - **Direção recomendada, ainda não implementada**: pra paciente já existente, em vez de ignorar — (1) "complementar" o cadastro: preencher só os campos que estão vazios na ficha atual, nunca substituir valor já preenchido; (2) anamnese nova **anexada** como um bloco datado no topo do texto existente, nunca substituindo o anterior — anamnese é registro clínico, perder observações da profissional não é aceitável. Isso muda o contrato de `POST /api/importacao/executar` (hoje só `create`) pra também tratar `update` condicional — fora do escopo do Bloco A/B, registrado aqui pra decisão futura do usuário.
