...
**Reprocessamento retroativo da anamnese — 2026-08-04**: a lógica de montagem do texto virou função exportada `montarAnamnese()` (refactor puro, sem mudança de comportamento — validado por comparação byte-a-byte contra a planilha real). `scripts/reprocessar-anamnese.ts` reaplica essa montagem retroativamente nos pacientes já importados antes da mudança acima, casando planilha↔paciente **somente por CPF**. Duas fases (`--fase=vazios|preenchidos`), sempre em dry-run por padrão (`--executar` obrigatório pra gravar, com backup automático antes de qualquer escrita). Nunca toca paciente sem CPF, CPF sem linha correspondente na planilha, ou CPF com mais de uma linha (repreenchimento — resolução manual). Na fase `preenchidos`, preserva byte a byte tudo a partir de `"--- OBSERVAÇÕES ---"` (com verificação em código antes de cada gravação); registro sem essa marca (formato antigo) não é tocado. Dry-run rodado contra `pamela-rachid` antes do commit — 4 elegíveis em cada fase, nenhuma escrita feita; execução real (`--executar`) aguarda autorização explícita separada. Detalhe técnico completo em `ARCHITECTURE.md` §9.

**PENDENTE DE DECISÃO — dedupe/complemento na importação (2026-08-04)**: hoje, paciente já existente (CPF já cadastrado) é **ignorado silenciosamente** pela importação — `POST /api/importacao/executar` só cria (`status: "novo"`), nunca atualiza. **Risco conhecido**: linha da planilha com CPF em branco nunca é marcada `"existente"` (o match exige `cpfDigitos.length > 0`), então reimportar a mesma pessoa sem CPF preenchido cria paciente duplicado — há registros reais nessa condição na base (49 pacientes sem CPF na Pâmela). **Direção recomendada, ainda não implementada**: para paciente existente, complementar o cadastro (só preencher campos vazios, nunca substituir valor já preenchido) e anexar anamnese nova como bloco datado no topo do texto existente, nunca substituindo — anamnese é registro clínico, perder observação da profissional não é aceitável.
