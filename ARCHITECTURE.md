...
| `ConviteClinica` | Convite de uso único para `POST /api/auth/signup` criar clínica nova (2026-09-17, ver §9) | `token` (único, aleatório via `crypto`), `email` (o cadastro só é aceito com esse e-mail, case-insensitive), `nomeClinicaSugerido?`, `expiraEm`, `usadoEm?`/`clinicaCriadaId?` (gravados juntos, na mesma transação que cria a `Clinica`) |
...
| POST | `/api/auth/signup` | `auth/signup/route.ts` | Cria clínica nova (vira ADMIN, exige `ConviteClinica` válido — 2026-09-17, ver §9) ou entra em clínica existente (exige ADMIN + `clinicaId` batendo, checado manualmente) | público (com convite) / manual |
| GET | `/api/auth/convite/validar` | `auth/convite/validar/route.ts` | Checa um `ConviteClinica` por `token` pra tela `/cadastro` — responde só `{ valido, email?, nomeClinicaSugerido? }`, nunca o motivo de uma falha (2026-09-17, ver §9) | público |
...
| POST | `/api/integracoes/google/avisar-responsavel` | `integracoes/google/avisar-responsavel/route.ts` | Notifica `WHATSAPP_TELEFONE_NOTIFICACAO_HUMANO` (mesmo canal de `notificarHandoffHumano`) que a conexão Google caiu — botão do popup pra quem não tem `gerirIntegracoes` (2026-08-14); sucesso silencioso (sem enviar) se a clínica do usuário logado não tiver WhatsApp configurado (2026-09-17, ver §9) | NONE (qualquer usuário logado) |
...
### `src/components/EmpurrarModal.tsx` (2026-08-20)
Modal de empurrar sessões futuras em N semanas (+ opcionalmente trocar dia da semana/horário), `POST /api/pacientes/[id]/empurrar` — extraído de JSX inline + 7 `useState` locais que viviam em `painel/page.tsx` até esta data, pra ser reutilizado também pelo botão "Empurrar" do card da agenda (`AgendaCalendario.tsx`, no `SessaoDetalheModal`). Diferente de `TarefaForm.tsx`, o estado interno (`semanas`, `mudarDiaHorario`, `novoDia`, `novoHorario`, `salvando`, `erro`) reseta sozinho — um `useEffect` disparado quando a prop `aberto` vira `true` — em vez de depender de troca de `key` por quem chama. Props: `pacienteId` (alvo da rota), `aberto`/`onFechar` (visibilidade), `onSucesso()` — chamado só depois do POST ter sucesso; **o componente nunca chama nenhuma função `carregarSessoes` diretamente**, decisão deliberada: existem duas funções homônimas com significado diferente (`painel/page.tsx` recarrega a lista de sessões do paciente selecionado; `AgendaCalendario.tsx` recarrega a grade do calendário por intervalo de datas) — cada tela passa a sua via `onSucesso`. Nenhuma regra de negócio migrou pra cá: validação de `semanas` (1-10), a regra de conflito de semana e o enfileiramento de sync com o Google continuam inteiramente em `empurrar/route.ts`, únicos e inalterados. A rota não tem conceito de "sessão de corte" (isso só existe em `adiar/route.ts`, "Trazer") — sempre move todas as sessões futuras não-canceladas do paciente, então o botão "Empurrar" no card afeta o pacote inteiro do paciente daquela sessão, não só o card clicado.
...
| `clinica/inicializar.ts` | `inicializarClinica(tx, clinicaId)` (2026-09-17) — dados mínimos pra uma `Clinica` nova funcionar (hoje: 1 `TipoSessao` padrão), chamada dentro da mesma transação que cria a `Clinica` em `POST /api/auth/signup`, ver §9 |
...
- **Feature nova (2026-09-17, terceiro bloco) — páginas públicas `/termos` e `/privacidade` + filtro de WhatsApp por clínica nos crons/notificação**:
  - `src/app/termos/page.tsx` e `src/app/privacidade/page.tsx` — conteúdo em pt-BR cobrindo o exigido pelo consentimento OAuth do Google e pelo checkbox de `/cadastro` (dados coletados, finalidade, base legal LGPD, papéis controladora/operadora — clínica controladora dos dados do paciente, Banah Digital operadora —, escopos do Google usados e compromisso de não vender/usar dado pra treinar IA, subprocessadores, armazenamento, retenção, direitos do titular, cookies, pedido de exclusão / descrição do serviço, acesso por convite, responsabilidades da clínica, indisponibilidade, limitação de responsabilidade, gratuidade com encerramento mediante aviso, suspensão por uso indevido, foro São Paulo/SP). Ambas têm aviso explícito de "modelo inicial, não revisado por advogado" e data de última atualização. Públicas por padrão (não passam pelo proxy, matcher só `/api/:path*`), mesmo mecanismo de `/cadastro`. Linkadas no rodapé de `/login` e `/cadastro`.
  - `cron/whatsapp-lembretes/route.ts` (`enviarLembretes48h` e `enviarMensagensDoDia`) passam a filtrar `paciente.clinica` por `CLINICA_TEM_WHATSAPP_CONFIGURADO = { conversasWhatsapp: { some: {} } }` — sem migration nem hardcode de slug, usa o sinal existente mais direto de "clínica já usa WhatsApp de verdade" (ter ao menos uma `ConversaWhatsapp` gravada). Confirmado por consulta ao banco em 2026-09-17: só `pamela-rachid` tem linhas em `ConversaWhatsapp` (53) — qualquer outra clínica (`clinica-teste` incluída) tem zero, então o comportamento da Pâmela **não muda** e nenhuma clínica nova recebe os dois envios automáticos deste cron pelo número global até organicamente ganhar sua primeira `ConversaWhatsapp` (hoje só via `POST /api/whatsapp/conversas`, fora do escopo deste ajuste).
  - `integracoes/google/avisar-responsavel/route.ts` — mesmo critério: se a clínica do usuário logado não tiver nenhuma `ConversaWhatsapp`, a rota responde `{ ok: true }` sem enviar nada (sucesso silencioso, só loga), em vez de notificar `WHATSAPP_TELEFONE_NOTIFICACAO_HUMANO` em nome de uma clínica sem WhatsApp.
  - Webhook, wa-bridge, provider, templates e `responderWhatsapp.ts` **não foram tocados** — continuam como documentado nas entradas anteriores.
  - **Módulo Mentoria — reconfirmado** (mesma checagem do bloco anterior, sem mudança): `mentoriaAtivada` só é alterável direto no banco, nenhuma rota/tela permite a um ADMIN ativá-la.
- **Feature nova (2026-09-17) — cadastro público de clínica nova (`POST /api/auth/signup`, caminho sem `clinicaId`) passa a exigir convite**: resposta aos achados 1/2/9/10 de `docs/auditorias/auditoria-onboarding.md` (rota aberta sem convite, sem rate limit, sem transação, e clínica nova sem `TipoSessao`). Model novo `ConviteClinica` (migration `20260917122549_convite_clinica`, puramente aditiva — só `CREATE TABLE` + FK opcional pra `Clinica`, nada alterado em tabela existente): `token` (32 bytes aleatórios via `crypto.randomBytes`, hex), `email` (cadastro só aceito com esse e-mail, comparação case-insensitive), `nomeClinicaSugerido?`, `expiraEm`, `usadoEm?`/`clinicaCriadaId?`. Convite é criado por `node scripts/gerar-convite.mjs --email=... [--clinica="..."] [--dry-run]`, validade 14 dias, imprime `{APP_URL}/cadastro?convite={token}`. **Atualização 2026-09-17 (mesmo dia, segundo bloco)** — tela pública `src/app/cadastro/page.tsx` construída: sem `?convite=` mostra "Link de convite necessário" (não chama API nenhuma); com token, `GET /api/auth/convite/validar?token=...` (rate limit próprio, mesmo padrão) confirma validade sem revelar o motivo de uma falha — responde só `{ valido, email?, nomeClinicaSugerido? }`; convite válido mostra formulário (nome, e-mail travado = e-mail do convite, nome da clínica pré-preenchido com `nomeClinicaSugerido` mas editável, senha + confirmação — mínimo 8 caracteres, igual `usuario/senha/route.ts` — e checkbox obrigatório de aceite dos Termos/Privacidade, linkando `/termos`/`/privacidade` — páginas construídas no bloco seguinte, ver entrada logo abaixo). Ambas as rotas novas (`/cadastro`, `/api/auth/convite/validar`) já nascem públicas sem qualquer mudança em `proxy.ts`/`middleware.ts`: `/cadastro` nunca passa pelo proxy (matcher só cobre `/api/:path*`) e `/api/auth/convite/validar` cai em `ehRotaAuth` (prefixo `/api/auth`), já excluído da exigência de sessão. `POST /api/auth/signup` passou a validar também `senha.length >= 8` no servidor (antes só confiava no mínimo do Supabase). `POST /api/auth/signup`: (1) rate limit por IP (`checkRateLimiteLocal`, 10/hora, mesmo padrão de `f/[clinicaSlug]/[formularioSlug]`); (2) valida o convite (existe, não expirado, não usado, e-mail bate) — qualquer falha responde só `"Convite inválido ou expirado"`, sem detalhar o motivo; (3) cria a conta no Supabase Auth; (4) numa única `prisma.$transaction`, cria `Clinica` + `Usuario` (ADMIN) + `inicializarClinica()` (§6, `src/lib/clinica/inicializar.ts` — hoje só 1 `TipoSessao` padrão, "Sessão individual"/50min; sem isso `POST /api/pacientes` rejeitava qualquer cadastro por falta de `tipoSessaoId`) + marca o convite usado via `updateMany({ where: { id, usadoEm: null }, ... })` (condicional, não por id direto — fecha a corrida entre duas requisições com o mesmo token: a segunda vê `count: 0` e a transação inteira dá rollback). Se qualquer parte da transação falhar, o `catch` remove a conta recém-criada no Supabase Auth (`admin.auth.admin.deleteUser`) — antes disso, uma falha depois do `Clinica.create` deixava clínica órfã sem admin; agora não sobra nem `Clinica` nem usuário Supabase. `emailBoasVindasAssunto`/`emailBoasVindasCorpo` da clínica nova são gravados explicitamente com o nome da clínica (não o `@default` do schema, que continua o texto assinado "Fono Pâmela Rachid" — inalterado, e a clínica `pamela-rachid` também não foi tocada). O caminho com `clinicaId` (ADMIN convidando colega pra própria clínica, único caller: `painel/configuracoes/seguranca/page.tsx`) não muda — continua sem convite, já exige sessão ADMIN válida. **Módulo Mentoria confirmado exclusivo da `pamela-rachid`**: `mentoriaAtivada` nasce `false` (default do schema, não tocado pelo signup) e não existe — nem existiu antes desta entrega — nenhuma rota/tela onde um ADMIN ligue essa flag sozinho (`PATCH /api/clinica` não inclui `mentoriaAtivada` em `CAMPOS_EDITAVEIS`); único jeito é update direto no banco pela equipe técnica.
