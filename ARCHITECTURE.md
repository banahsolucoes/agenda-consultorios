...
| GET | `/api/cron/sincronizacao` | `cron/sincronizacao/route.ts` | Worker do outbox de sincronização Google — `processarPendentes()` (2026-08-14, ver §9) — protegida por `CRON_SECRET` | `CRON_SECRET` |
...
- **Feature nova (2026-08-14) — outbox de sincronização Google (Calendar/Drive), retry com backoff, idempotência**. Motivada pela auditoria que confirmou o padrão estrutural por trás de vários incidentes acima (Jadir, Guilherme/Felipe): toda chamada Google era síncrona e best-effort — falha virava, no máximo, um `console.error` e (só nos 6 catches cobertos) `googleTokenValido=false`; não havia retry automático nem sinal visível pro operador além do banner de token revogado.
  ...
  - **UI**: `painel/page.tsx` — badge "Sincronizando…" ao lado do status da sessão quando `Agendamento.googleSyncStatus === "PENDENTE"` (campo já vinha na resposta de `GET /api/agendamentos`, só não estava no tipo `Sessao` nem lido); mensagem de copiar Meet troca "(link ainda não gerado)" por "(sincronizando com o Google...)" nesse estado. `painel/configuracoes/integracoes/page.tsx` — contador de itens `FALHA` (via `itensFalha` em `GET /api/integracoes/google/status`) com botão "Tentar de novo" (`POST /api/integracoes/google/reenfileirar`, reseta pra `PENDENTE`/tentativas `0`/`ultimoErro null`, `gerirIntegracoes`). Banner de `painel/page.tsx` (texto genérico "algo falhou na sincronização", não mais só "conexão perdida") passa a reagir também a `googleUltimoErroEm` dentro de uma janela de 2h (`JANELA_ERRO_RECENTE_MS`, `notificacoes/route.ts`), não só `googleTokenValido=false`.
  - **`GET /api/cron/sincronizacao`** — mesmo mecanismo de auth de `verificar-google-noturno` (`CRON_SECRET`), processa até 25 itens por execução (`LIMITE_POR_EXECUCAO`). Registrado em `vercel.json` como `*/10 * * * *`. **Risco de deploy não resolvido nesta feature**: a entrada de 2026-07-24 acima registra que o cron diário já foi projetado em torno do limite do plano Vercel Hobby ("no máximo 1x/dia" por cron); Hobby também limita a **2 cron jobs no total**, e este projeto já tinha 2 (`verificar-google-noturno` + `whatsapp-lembretes`) — um 3º cron, e num intervalo de 10 minutos (não diário), muito provavelmente excede os dois limites do Hobby simultaneamente. Não verificado nesta sessão se o projeto já está em plano Pro; se não estiver, o deploy deste cron especificamente tende a falhar ou ser rejeitado pela Vercel até o upgrade.
