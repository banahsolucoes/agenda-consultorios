...
**Auditoria de performance 2026-07-17**: corrigidos os achados 1 (waterfall do guard de navegação) e 3 (awaits sequenciais + somas em JS no dashboard) — `src/app/mentoria/layout.tsx` agora usa um único fetch enxuto (`GET /api/mentoria/acesso`, novo, reaproveita `exigirAcessoMentoria`) em vez de buscar `/api/clinica` inteira; `dashboard/resumo`, `dashboard/mensal` e `calcularAgregadosMensais`/`calcularInadimplenciaAtual` (`src/lib/mentoria.ts`) trocaram `await` sequencial e `findMany`+soma em JS por `Promise.all`/`prisma.aggregate`; `dashboard/page.tsx` ganhou estado único de carregamento com skeleton e fail-safe, mesmo padrão de `painel/page.tsx`. `calcularImpostoNoMes`/`calcularComissaoNoMes`/`calcularComissaoPendenteNoMes` ficaram de fora por serem multiplicativas (linha × taxa × percentual) — não expressáveis em `aggregate`/`groupBy` do Prisma sem SQL bruto. Achado 2 (PII em `include`) e índices/paginação não foram tocados nesta rodada. Ver relatório completo em `Documentos Claude/auditoria-mentoria-2026-07-17.md`.

**Auditoria de performance 2026-07-17 (achado 2)**: reduzido over-fetching de PII em `GET /api/mentoria/alunos` (lista — `include` virou `select`, só `id`/`nomeCompleto`/`dataNascimento`/contrato ativo; CPF/RG/endereço/contato saíram da resposta) e `GET /api/mentoria/contratos/[id]` (`aluno: true` virou `aluno: { select: { id, nomeCompleto } }`). Mapeamento de consumidores confirmou um único chamador de cada rota, nenhum lendo os campos removidos. Cadastro completo do aluno continua disponível em `GET /api/mentoria/alunos/[id]` (rota de detalhe já existente, não alterada) — mesmo padrão lista-enxuta/detalhe-completo já usado em Pacientes do Consultório. Ver relatório completo em `Documentos Claude/auditoria-mentoria-2026-07-17.md`.
