...
- O Google Calendar continua recebendo o **espelho** (escrita a partir do banco), para quem quiser ver no Google. Mas a **edição oficial** passa a ser no painel. O espelho vale para **qualquer tipo de sessão** (presencial ou online) quando a clínica está conectada — até 2026-07-21 havia um bug em que sessão presencial nunca era espelhada (só online gerava evento), corrigido nessa data. Cada `Agendamento` grava um `googleSyncStatus` (sincronizado/falhou/não aplicável) para dar visibilidade de quando o espelho realmente aconteceu.
